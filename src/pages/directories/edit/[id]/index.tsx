...
import { getDirectoryById, updateDirectoryById } from 'apiSdk/directories';
...
function DirectoryEditPage() {
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<DirectoryInterface>(
    () => (id ? `/directories/${id}` : null),
    () => getDirectoryById(id),
  );
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: DirectoryInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateDirectoryById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<DirectoryInterface>({
    initialValues: data,
