...
import { getDirectories } from 'apiSdk/directories';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['creator', 'admin', 'content-moderator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<DirectoryInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/directories` : null),
    () => getDirectories({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Directory',
      path: isTenantUser ? `/directories/view/${data?.[0]?.id}` : '/directories',
      entity: 'directory',
